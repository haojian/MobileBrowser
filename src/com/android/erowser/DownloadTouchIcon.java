...
class DownloadTouchIcon extends AsyncTask<String, Void, Bitmap> {
    ...
    /* package */ ErowserActivity mActivity;

    public DownloadTouchIcon(ErowserActivity activity, ContentResolver cr,
    ...
    public void onPostExecute(Bitmap icon) {
        ...
        values.put(ErowserP.BookmarkColumns.TOUCH_ICON,
                os.toByteArray());

        if (mCursor.moveToFirst()) {
            do {
                mContentResolver.update(ContentUris.withAppendedId(
                        ErowserP.BOOKMARKS_URI, mCursor.getInt(0)),
                        values, null, null);
