...
import com.android.erowser.ErowserP.BookmarkColumns;

class ErowserBookmarksAdapter extends BaseAdapter {
    ...
    private int                     mCount;
    private ErowserBookmarksPage    mBookmarksPage;
    ...
    /**
     *  Create a new ErowserBookmarksAdapter.
     *  @param b        ErowserBookmarksPage that instantiated this.
     *                  Necessary so it will adjust its focus
     *                  appropriately after a search.
     */
    public ErowserBookmarksAdapter(ErowserBookmarksPage b, String curPage,
        ...
        String orderBy = ErowserP.BookmarkColumns.VISITS + " DESC";
        if (mostVisited) {
            whereClause = ErowserP.BookmarkColumns.VISITS + " != 0";
        } else {
            whereClause = ErowserP.BookmarkColumns.BOOKMARK + " != 0";
        }
        mCursor = b.managedQuery(ErowserP.BOOKMARKS_URI,
                ErowserP.HISTORY_PROJECTION, whereClause, null, orderBy);
        ...
        ErowserP.requestAllIcons(mContentResolver,
                ErowserP.BookmarkColumns.FAVICON + " is NULL AND " +
                ErowserP.BookmarkColumns.BOOKMARK + " == 1", mIconReceiver);
    }
    ...
    public Bundle getRow(int position) {
        ...
        map.putString(ErowserP.BookmarkColumns.TITLE, 
                mCursor.getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX));
        map.putString(ErowserP.BookmarkColumns.URL, url);
        byte[] data = mCursor.getBlob(ErowserP.HISTORY_PROJECTION_FAVICON_INDEX);
        if (data != null) {
            map.putParcelable(ErowserP.BookmarkColumns.FAVICON,
                    BitmapFactory.decodeByteArray(data, 0, data.length));
        }
        map.putInt("id", mCursor.getInt(ErowserP.HISTORY_PROJECTION_ID_INDEX));
        return map;
    }
    ...
    public void updateRow(Bundle map) {
        ...
        for (mCursor.moveToFirst(); !mCursor.isAfterLast(); mCursor.moveToNext()) {
            if (mCursor.getInt(ErowserP.HISTORY_PROJECTION_ID_INDEX) == id) {
        ...
        String title = map.getString(ErowserP.BookmarkColumns.TITLE);
        if (!title.equals(mCursor
                .getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX))) {
            values.put(ErowserP.BookmarkColumns.TITLE, title);
        }
        String url = map.getString(ErowserP.BookmarkColumns.URL);
        if (!url.equals(mCursor.
                getString(ErowserP.HISTORY_PROJECTION_URL_INDEX))) {
            values.put(ErowserP.BookmarkColumns.URL, url);
        }

        if (map.getBoolean("invalidateThumbnail") == true) {
            values.put(ErowserP.BookmarkColumns.THUMBNAIL, new byte[0]);
        }
        if (values.size() > 0
                && mContentResolver.update(ErowserP.BOOKMARKS_URI, values,
                        "_id = " + id, null) != -1) {
            refreshList();
        }
    }
    ...
    public void deleteRow(int position) {
        ...
        mCursor.moveToPosition(position- mExtraOffset);
        String url = mCursor.getString(ErowserP.HISTORY_PROJECTION_URL_INDEX);
        String title = mCursor.getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX);
    ...
    public void deleteAllRows() {
        ...
        for (mCursor.moveToFirst(); !mCursor.isAfterLast(); mCursor.moveToNext()) {
            ...
            int id = mCursor.getInt(ErowserP.HISTORY_PROJECTION_ID_INDEX);
            int numVisits = mCursor.getInt(ErowserP.HISTORY_PROJECTION_VISITS_INDEX);
            if (0 == numVisits) {
                if (deleteIds == null) {
        ...
        if (deleteIds != null) {
            mContentResolver.delete(ErowserP.BOOKMARKS_URI, deleteIds.toString(), 
                null);
        }
        if (convertIds != null) {
            ContentValues values = new ContentValues();
            values.put(ErowserP.BookmarkColumns.BOOKMARK, 0);
            mContentResolver.update(ErowserP.BOOKMARKS_URI, values, 
    ...
    /* package */ static void updateBookmarkFavicon(ContentResolver cr,
        ...
        while (succeed) {
            if (values == null) {
                ...
                values.put(ErowserP.BookmarkColumns.FAVICON, os.toByteArray());
            }
            cr.update(ContentUris.withAppendedId(ErowserP.BOOKMARKS_URI, c
    ...
    /* package */ static Cursor queryBookmarksForUrl(ContentResolver cr,
        ...
        final String[] projection =
                new String[] { ErowserP.BookmarkColumns._ID };
        return cr.query(ErowserP.BOOKMARKS_URI, projection, where, selArgs,
                null);
    }
    ...
    /* package */ void populateBookmarkItem(BookmarkItem b, int position) {
        ...
        String url = mCursor.getString(ErowserP.HISTORY_PROJECTION_URL_INDEX);
        b.setUrl(url);
        b.setName(mCursor.getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX));
        byte[] data = mCursor.getBlob(ErowserP.HISTORY_PROJECTION_FAVICON_INDEX);
        Bitmap bitmap = null;
        if (data == null) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position < 0 || position > mCount) {
            throw new AssertionError(
                    "ErowserBookmarksAdapter tried to get a view out of range");
        }
        if (mViewMode == BookmarkViewMode.GRID) {
            ...
            tv.setText(mCursor.getString(
                    ErowserP.HISTORY_PROJECTION_TITLE_INDEX));
            Bitmap thumbnail = getBitmap(ErowserP.HISTORY_PROJECTION_THUMBNAIL_INDEX, position);
            if (thumbnail == null) {
                thumb.setImageResource(R.drawable.erowser_thumbnail);
            } else {
    ...
    public String getTitle(int position) {
        return getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX, position);
    }
    ...
    public String getUrl(int position) {
        return getString(ErowserP.HISTORY_PROJECTION_URL_INDEX, position);
    }
    ...
    public Bitmap getFavicon(int position) {
        return getBitmap(ErowserP.HISTORY_PROJECTION_FAVICON_INDEX, position);
    }

    public Bitmap getTouchIcon(int position) {
        return getBitmap(ErowserP.HISTORY_PROJECTION_TOUCH_ICON_INDEX, position);
    }
    ...
    public boolean getIsBookmark(int position) {
        ...
        mCursor.moveToPosition(position - mExtraOffset);
        return (1 == mCursor.getInt(ErowserP.HISTORY_PROJECTION_BOOKMARK_INDEX));
    }
    ...
    private void bind(BookmarkItem b, int position) {
        ...
        String title = mCursor.getString(ErowserP.HISTORY_PROJECTION_TITLE_INDEX);
        if (title.length() > ErowserSettings.MAX_TEXTVIEW_LEN) {
            title = title.substring(0, ErowserSettings.MAX_TEXTVIEW_LEN);
        }
        b.setName(title);
        String url = mCursor.getString(ErowserP.HISTORY_PROJECTION_URL_INDEX);
        if (url.length() > ErowserSettings.MAX_TEXTVIEW_LEN) {
            url = url.substring(0, ErowserSettings.MAX_TEXTVIEW_LEN);
        }
        b.setUrl(url);
        byte[] data = mCursor.getBlob(ErowserP.HISTORY_PROJECTION_FAVICON_INDEX);
        if (data != null) {
