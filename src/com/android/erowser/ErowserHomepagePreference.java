...
public class ErowserHomepagePreference extends EditTextPreference {

    public ErowserHomepagePreference(Context context, AttributeSet attrs,
            int defStyle) {
    ...
    public ErowserHomepagePreference(Context context) {
        super(context);
    }
    ...
    protected void onDialogClosed(boolean positiveResult) {
        if (positiveResult) {
            ...
            if (url.length() > 0
                    && !ErowserActivity.ACCEPTED_URI_SCHEMA.matcher(url)
                            .matches()) {
