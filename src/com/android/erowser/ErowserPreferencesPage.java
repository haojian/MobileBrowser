...
public class ErowserPreferencesPage extends PreferenceActivity
        implements Preference.OnPreferenceChangeListener {

    private String LOGTAG = "ErowserPreferencesPage";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.erowser_preferences);

        Preference e = findPreference(ErowserSettings.PREF_HOMEPAGE);
        e.setOnPreferenceChangeListener(this);
        e.setSummary(getPreferenceScreen().getSharedPreferences()
                .getString(ErowserSettings.PREF_HOMEPAGE, null));
        
        e = findPreference(ErowserSettings.PREF_EXTRAS_RESET_DEFAULTS);
        e.setOnPreferenceChangeListener(this);
        
        e = findPreference(ErowserSettings.PREF_TEXT_SIZE);
        e.setOnPreferenceChangeListener(this);
        e.setSummary(getVisualTextSizeName(
                getPreferenceScreen().getSharedPreferences()
                .getString(ErowserSettings.PREF_TEXT_SIZE, null)) );
        
        e = findPreference(ErowserSettings.PREF_DEFAULT_ZOOM);
        e.setOnPreferenceChangeListener(this);
        e.setSummary(getVisualDefaultZoomName(
                getPreferenceScreen().getSharedPreferences()
                .getString(ErowserSettings.PREF_DEFAULT_ZOOM, null)) );

        e = findPreference(ErowserSettings.PREF_DEFAULT_TEXT_ENCODING);
        e.setOnPreferenceChangeListener(this);

        if (ErowserSettings.getInstance().showDebugSettings()) {
            addPreferencesFromResource(R.xml.debug_preferences);
        }

        PreferenceScreen websiteSettings = (PreferenceScreen)
            findPreference(ErowserSettings.PREF_WEBSITE_SETTINGS);
    ...
    protected void onPause() {
        ...
        // sync the shared preferences back to ErowserSettings
        ErowserSettings.getInstance().syncSharedPreferences(
                getPreferenceScreen().getSharedPreferences());
    }

    public boolean onPreferenceChange(Preference pref, Object objValue) {
        if (pref.getKey().equals(ErowserSettings.PREF_EXTRAS_RESET_DEFAULTS)) {
            Boolean value = (Boolean) objValue;
            if (value.booleanValue() == true) {
                finish();
            }
        } else if (pref.getKey().equals(ErowserSettings.PREF_HOMEPAGE)) {
            ...
            EventMonitor.getInstance().sendSetHomePage(value);//shuaiyuan added
        ...
        } else if (pref.getKey().equals(ErowserSettings.PREF_TEXT_SIZE)) {
            pref.setSummary(getVisualTextSizeName((String) objValue));
            return true;
        } else if (pref.getKey().equals(ErowserSettings.PREF_DEFAULT_ZOOM)) {
            pref.setSummary(getVisualDefaultZoomName((String) objValue));
            return true;
        } else if (pref.getKey().equals(
                ErowserSettings.PREF_DEFAULT_TEXT_ENCODING)) {
