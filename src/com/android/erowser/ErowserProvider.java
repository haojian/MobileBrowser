...
public class ErowserProvider extends ContentProvider {

    private SQLiteOpenHelper mOpenHelper;
    private BackupManager mBackupManager;
    private static final String sDatabaseName = "erowser.db";
    private static final String TAG = "ErowserProvider";
    ...
    static {
        ...
        URI_MATCHER.addURI("erowser", TABLE_NAMES[URI_MATCH_BOOKMARKS],
                URI_MATCH_BOOKMARKS);
        URI_MATCHER.addURI("erowser", TABLE_NAMES[URI_MATCH_BOOKMARKS] + "/#",
                URI_MATCH_BOOKMARKS_ID);
        URI_MATCHER.addURI("erowser", TABLE_NAMES[URI_MATCH_SEARCHES],
                URI_MATCH_SEARCHES);
        URI_MATCHER.addURI("erowser", TABLE_NAMES[URI_MATCH_SEARCHES] + "/#",
                URI_MATCH_SEARCHES_ID);
        URI_MATCHER.addURI("erowser", SearchManager.SUGGEST_URI_PATH_QUERY,
                URI_MATCH_SUGGEST);
        URI_MATCHER.addURI("erowser",
    ...
    // 14 -> 15 move couple of tables to either erowser private database or webview database
        ...
        private void removeGears() {
            AsyncTask<Void, Void, Void> task = new AsyncTask<Void, Void, Void>() {
                public Void doInBackground(Void... unused) {
                    String erowserDataDirString = mContext.getApplicationInfo().dataDir;
                    final String appPluginsDirString = "app_plugins";
                    final String gearsPrefix = "gears";
                    File appPluginsDir = new File(erowserDataDirString + File.separator
                            + appPluginsDirString);
                    ...
                    File gearsDataDir = new File(erowserDataDirString + File.separator
                            + gearsPrefix);
    ...
    private class MySuggestionCursor extends AbstractCursor {
                ...
                switch(columnIndex) {
                    ...
                    case SUGGEST_COLUMN_QUERY_ID:
                        if (mHistoryCount > mPos) {
                            ...
                            // within the erowser because our searchable is set to
        ...
        /**
         * Provides the title (text line 1) for a erowser suggestion, which should be the
        ...
        /**
         * Provides the subtitle (text line 2) for a erowser suggestion, which should be the
        ...
        switch (match) {
            case URI_MATCH_BOOKMARKS: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_BOOKMARKS], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(ErowserP.BOOKMARKS_URI,
                            rowID);
                }
            ...
            case URI_MATCH_SEARCHES: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_SEARCHES], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(ErowserP.SEARCHES_URI,
                            rowID);
                }
                break;
            }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        if (isBookmarkTable) {
            Cursor cursor = cr.query(ErowserP.BOOKMARKS_URI,
