...
public class TitleBar extends LinearLayout {
    ...
    private ErowserActivity mErowserActivity;
    ...
    public TitleBar(ErowserActivity context) {
        super(context, null);
        mHandler = new MyHandler();
        LayoutInflater factory = LayoutInflater.from(context);
        factory.inflate(R.layout.title_bar, this);
        mErowserActivity = context;
    ...
    private class MyHandler extends Handler {
        public void handleMessage(Message msg) {
            if (msg.what == LONG_PRESS) {
                ...
                // Need to call a special method on ErowserActivity for when the
                // fake title bar is up, because its ViewGroup does not show a
                // context menu.
                mErowserActivity.showTitleBarContextMenu();
            }
        }
    };
    ...
    protected void onCreateContextMenu(ContextMenu menu) {
        MenuInflater inflater = mErowserActivity.getMenuInflater();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                int slop = ViewConfiguration.get(mErowserActivity)
                        .getScaledTouchSlop();
            ...
            case MotionEvent.ACTION_UP:
                if (mRtButton.isPressed()) {
                    if (mInLoad) {
                        mErowserActivity.stopLoading();
                    } else {
                        mErowserActivity.bookmarksOrHistoryPicker(false);
                    }
                    mRtButton.setPressed(false);
                } else if (mTitleBg.isPressed()) {
                    mHandler.removeMessages(LONG_PRESS);
                    mErowserActivity.onSearchRequested();
                    mTitleBg.setPressed(false);
                }
                break;
            default:
