...
public class WebsiteSettingsActivity extends ListActivity {
    ...
    class SiteAdapter extends ArrayAdapter<Site>
        ...
        public void populateIcons(Map<String, Site> sites) {
            ...
            Cursor c = getContext().getContentResolver().query(ErowserP.BOOKMARKS_URI,
                    new String[] { ErowserP.BookmarkColumns.URL, ErowserP.BookmarkColumns.TITLE,
                    ErowserP.BookmarkColumns.FAVICON }, "bookmark = 1", null, null);

            if (c != null) {
                if(c.moveToFirst()) {
                    int urlIndex = c.getColumnIndex(ErowserP.BookmarkColumns.URL);
                    int titleIndex = c.getColumnIndex(ErowserP.BookmarkColumns.TITLE);
                    int faviconIndex = c.getColumnIndex(ErowserP.BookmarkColumns.FAVICON);
                    do {
